    ...
    public partial class MainView : UserControl
    {
        ...
        private void MainCanvas_SizeChanged(object? sender, SizeChangedEventArgs e)
        {
            ...
            if (e.NewSize.Height > 650)
            {
                ...
                Task.Delay(15).ContinueWith((completedTask) =>
                {
        ...
        private async void ModifyButton_Click(object? sender, RoutedEventArgs e)
        {
            ToggleHighlightForLeafNodes(false);
            ToggleDetailsAndTraversePanels(false);
            ModifyNodeDialog modifyNodeDialog = new ModifyNodeDialog();
        ...
        private void HandleTraversing_Checked(object? sender, RoutedEventArgs e)
        {
            ToggleDetailsAndTraversePanels(true);
        }

        private void HandleTraversing_Unchecked(object? sender, RoutedEventArgs e)
        {
            ToggleDetailsAndTraversePanels(false);
        }

        private void HighlightLeafNodes_Checked(object? sender, RoutedEventArgs e)
        {
            ToggleHighlightForLeafNodes(true);
        }

        private void HighlightLeafNodes_Unchecked(object? sender, RoutedEventArgs e)
        {
            ToggleHighlightForLeafNodes(false);
        }
        #endregion
        ...
        private async Task AnimateNodesAsync(List<int> keys, TreeTraversalMode mode)
        {
            ...
            {
                Core.BinarySearchTreeNode? node = CanvasController.PhysicalItems.Search(key);
                if (node != null)
                {
        ...
        private void ToggleHighlightForLeafNodes(bool shouldHighlight)
        {
            ...
            foreach (int nodeKey in allNodesKeys)
            {
                ...
                {
                    TreeNode? ellipse = (node?.Data[CanvasController.DataProps[DataNodeProperties.PhysicalNode]]) as TreeNode;
